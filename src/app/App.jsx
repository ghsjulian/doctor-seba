...
Downloaded: ${new Date().toLocaleString('bn-BD')}
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center p-4 sm:p-8 font-sans"
      style={{ background: 'linear-gradient(135deg, #c8e6c9 0%, #a5d6a7 50%, #81c784 100%)' }}
    >
      {/* Phone Frame */}
      <div
        className="w-full max-w-[390px] h-[820px] max-h-[92vh] bg-[#F2F5F3] rounded-[2.8rem] overflow-hidden flex flex-col relative border-[10px] border-gray-900"
        style={{ boxShadow: '0 32px 80px -12px rgba(0,0,0,0.5), 0 0 0 1px rgba(0,0,0,0.1)' }}
      >
        {/* Notch */}
        ...
        >
          {showSettings ? (
            <SettingsPage lang={lang} onBack={() => setShowSettings(false)} />
          ) : activeTab === 'home' ? (
            <HomeTab lang={lang} />
